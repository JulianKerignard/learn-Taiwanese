...
export default function RevisionContent({
  ...
  const ctx: RevisionContext = { code: language.code, lessons };

  const stats = getStats(cards);
  const recommended = getRecommendedTopics(ctx, cards);

  const groupedData: TopicGroup[] =
    view === "unit" ? groupCardsByUnit(ctx, cards) :
    view === "chapter" ? groupCardsByChapter(ctx, cards) :
    view === "level" ? groupCardsByLevel(ctx, cards) :
    [];
    ...
    <div className="flex flex-col gap-10">
      ...
      {!hasCards && (
        <div className="card text-center py-12">
          ...
          <h2 className="text-title font-bold text-stone-800 mb-2">Aucune carte à réviser</h2>
          <p className="text-stone-500 mb-6">
            Ajoute du vocabulaire depuis les leçons ou le parcours pour commencer tes révisions.
          </p>
          <div className="flex justify-center gap-3">
            <Link href={langHref(lang, "/path")} className="btn-primary gap-1">
              <GraduationCap className="h-4 w-4" />
              Parcours
            </Link>
            <Link href={langHref(lang, "/lessons")} className="btn-secondary gap-1">
        ...
        <>
          ...
          <div className="flex gap-1 rounded-lg border border-stone-200 bg-stone-50 p-1">
            {([
              ...
              { key: "level" as const, label: `Par ${language.levels.code}` },
          ...
          {view === "recommended" && (
            <div className="flex flex-col gap-6">
              {recommended.length > 0 ? (
                <>
                  <h2 className="text-title font-bold text-stone-800">À réviser en priorité</h2>
                  <div className="grid gap-4 sm:grid-cols-2">
                    {recommended.map((topic) => (
                      <TopicCard key={topic.id} lang={lang} topic={topic} />
                    ))}
                  </div>
                </>
              ) : (
              ...
              {stats.due > 0 && (
                <Link
                  href={langHref(lang, "/revision/all")}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {groupedData.map((group) => (
                <TopicCard
                  key={group.id}
                  lang={lang}
                  topic={{ ...group, score: group.dueCount, type: "unit" }}
                />
              ))}
...
function TopicCard({
  ...
  lang: LanguageSegment;
  topic: RevisionTopic | (TopicGroup & { score: number; type: string });
}) {
  ...
  return (
    <Link
      href={langHref(lang, `/revision/${topic.id}`)}
