...
function generateQuestion(allWords: GameWord[], usedSet: Set<string>): Question | null {
  ...
  const options = [...distractors, word.french].sort(() => Math.random() - 0.5);
...
const DURATION = 60;
// The key is scoped per edition by storageKey(): a Japanese record never
// overwrites a Mandarin one.
const STORAGE_KEY = KEYS.speedRecord;

// The word pool arrives as a prop: see ./page.tsx.
export default function SpeedQuizContent({
  lang,
  title,
  words: allWords,
}: {
  ...
  if (phase === "ready") {
    return (
      <div className="mx-auto max-w-xl py-4 text-center">
        ...
        <h1 className="text-display font-bold mb-4 text-stone-900">{title}</h1>
        <p className="mb-8 text-stone-500">
          Traduis le plus de mots possible en {DURATION} secondes !
        </p>
    ...
    return (
      <div className="mx-auto max-w-xl py-4 text-center">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8">
          ...
          <div className="mb-6 text-5xl font-bold text-stone-900">{score} pts</div>
          {isNewRecord && (
            <p className="mb-4 text-lg font-bold text-amber-600">Nouveau record !</p>
          )}
      ...
      {question && (
        <>
          <div className="mb-8 text-center">
            ...
            {showRomanization && (
              <p className="mt-2 text-sm text-stone-500 italic">{question.word.romanization}</p>
            )}
          </div>
