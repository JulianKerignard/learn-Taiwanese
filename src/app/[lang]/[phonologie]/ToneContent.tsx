...
import type { TonePair, TonePairWord, SandhiRule } from "@/data/zh/tone-pairs";
...
function ToneGrid({
  ...
  const mainPairs = pairs.filter((p) => p.tone1 > 0 && p.tone2 > 0);
  const neutralPairs = pairs.filter((p) => p.tone2 === 0);
...
function Exercise({
  ...
  pair: TonePair;
  /** Every pair of the edition, used to build the distractors. */
  allPairs: TonePair[];
  mode: ExerciseMode;
...
function SandhiSection({ rules }: { rules: SandhiRule[] }) {
  return (
    <div className="space-y-6">
      <div>
...
/**
 * The Mandarin phonology module: four syllabic tones, the 4x4 pair grid and the
 * sandhi rules.
 *
...
export default function ToneContent({
  ...
  return (
    <div className="w-full mx-auto max-w-3xl space-y-6">
      ...
      {view === "sandhi" && <SandhiSection rules={sandhiRules} />}

      {view === "grid" && !selectedPair && (
        <ToneGrid pairs={tonePairs} progress={progress} onSelect={handleSelectPair} />
      )}
